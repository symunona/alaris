...
try {
	...
	exports.db = db = { blog: [], tags: [], events: [] }
}
...
exports.deleteItem = function (table, id) {
	...
	if (element) {
		...
		console.log('removing', table, id, element)
		exports.persist()
	}
	else {
		console.warn('entry does not exists', id)
		throw new Error('entry does not exists with id ' + id)
	}
}
...
exports.saveOrUpdate = function (table, entry) {
	...
	if (!entry.id) {
		entry.id = ++maxBlogId
		// default create time only if missing
		if (!entry.date) {
			entry.date = moment().format()
		}
	}
	...
	if (existing) {
		...
		if (!exports.db[table]) exports.db[table] = []
